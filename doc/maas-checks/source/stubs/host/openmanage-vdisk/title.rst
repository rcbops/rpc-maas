The *openmanage vdisk* check utilizes a custom plugin, `openmanage.py
...
The plugin executes ``omreport storage vdisk`` and returns a boolean
metric with the following values:
...
``openmanage`` plugin scoped to storage:

.. code-block:: console

    root@infra01:~# /usr/lib/rackspace-monitoring-agent/plugins/run_plugin_in_venv.sh \
      /usr/lib/rackspace-monitoring-agent/plugins/openmanage.py storage vdisk
    status okay
    metric hardware_vdisk_status uint32 1
