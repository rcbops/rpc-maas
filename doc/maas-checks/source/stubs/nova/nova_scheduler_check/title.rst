...
The plugin runs locally from the each ``nova_scheduler`` inventory node.
This check ensures the ``nova-scheduler`` service is operationally
healthy across all hosts.

Alarms associated with this monitoring check utilize the
``nova-scheduler_status`` metric, which is a string:
