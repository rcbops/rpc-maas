...
``neutron_service_check.py`` plugin run across ``neutron_metering_agent``
nodes. If the ``neutron-metering-agent_status`` metric is ``No`` for three
successive intervals, a critical alarm notification is generated. This
indicates the Neutron metering agent is down on the associated backend
container or host.
