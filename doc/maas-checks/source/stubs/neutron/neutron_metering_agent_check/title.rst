...
The plugin runs locally from the each ``neutron_metering_agent``
inventory node. This check provides granularity at an agent level to
ensure the Neutron metering agents are operationally healthy across all
backends.

Alarms associated with this monitoring check utilize the
``neutron-metering-agent_status`` metric, which is a string:
