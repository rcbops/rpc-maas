...
# Rally is excluded intentionally.
IGNORED_TEMPLATES = (
    "checks_base.yaml.j2",
    "clouds.yaml.j2",
    "maas_rally.yaml.j2",
    "network_throughput.yaml.j2",
    "neutron_metadata_proxy_check.yaml.j2",
    "nova_cloud_stats_check.yaml.j2",
    "rally_check.yaml.j2",
...
def _get_defaults(root=PLAYBOOKS_DIR, vars_dir=VARS_DIR):
    ...
    variables["maas_swift_accesscheck_container"] = \
...
def _get_globals(config_variables):
    ...
    # devices. This list is generated by ansible listing actual
    ...
    # host IDs.
    new_globals["ceph_osd_host"] = {'osd_id': "id"}
    # This is needed to populate bonding_iface alarms
    new_globals["maas_bonding_interfaces"] = ["<interface>"]
    # This is needed to populate the nova console type
    new_globals["console_service_name"] = "<console_type>"
    ...
    # Define swift dynamic variables
    new_globals["maas_swift_accesscheck_container"] = \
        config_variables["maas_swift_accesscheck_container"]
    new_globals["hostvars"] = {
        "groups": {
            "swift_proxy": [
                {
                    "maas_swift_access_url_key":
                        config_variables["maas_swift_access_url_key"]
                }
            ]
        }
    }
    ...
    # There are a few labels that use `item` from with_items or with_together,
    # and others that use a string. Use the following to fake them out allowing
    ...
    # Create a known good copy of the 'item' dictionary after building rally
    # values
    new_globals["dict_item"] = new_globals["item"]
    return new_globals
...
def _get_check_details(rendered_yaml, config_variables, environment):
    ...
    default value"""
    # Ignore maas_alarm_local_consecutive_count because it's not
    # something that should generally be changed, especially
    # because it has an effect on SLAs.
    ignore = ("maas_alarm_local_consecutive_count",
              "volume_group",
...
def _check_details(root=PLAYBOOKS_DIR, templates_dir=TEMPLATES_DIR):
    ...
    for template_file in rax_maas_path.glob("*.yaml.j2"):
        ...
        # NOTE(npawelek): Some check templates using the 'with_items' iterator
        # use a string and others use dictionaries. This conditional provides
        # the ability to specify a 'item' value wanted for display.
        if template_file.name == 'ceph_osd_stats.yaml.j2':
            env.globals.update(item="<id>")
        elif template_file.name == 'cinder_volume_check.yaml.j2':
            env.globals.update(item="<backend>")
        else:
            env.globals.update(item=env.globals["dict_item"])
